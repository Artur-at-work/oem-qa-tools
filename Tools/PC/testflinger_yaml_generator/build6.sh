...
if [ -n "$ADDITIONAL_PPAS" ]; then
	#cat /dev/null > template/shell_scripts/02_Install_checkbox_deb_2_ppa
...
echo "Generated $JOB_YAML content by $YAML_GENERATOR_EXE:==="
cat "$JOB_YAML"
echo "=== end ==="
rm -rf artifacts
...
GIT_SSH_COMMAND="ssh -o StrictHostKeyChecking=no" git clone git+ssh://oem-taipei-bot@git.launchpad.net/~lyoncore-team/lyoncore/+git/oem-tool -b master --depth 1
